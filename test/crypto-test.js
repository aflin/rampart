...
testFeature("unix passwd", function(){
    var ret=true;
    var pw = crypto.passwd("mypasswd", null, "md5");

    var line="$1$epCdp7c5$QROtiBBauvN/HPi5e22ty1";
    var c = crypto.passwdComponents(line);
    ret = ret && c.line==line && c.hash=='QROtiBBauvN/HPi5e22ty1'
              && c.salt=='epCdp7c5' && c.mode=='md5';

    if(crypto.passwdCheck(pw.line, "mypasswd"))
        return ret;
    else
        return false;
});

/* Cover every mode of crypto.passwd (md5crypt, apr1, aixmd5, sha256crypt,
 * sha512crypt, DES crypt).  The code that backs these was copied from
 * openssl 1.1.1's apps/passwd.c; these tests verify it still works on
 * the OpenSSL 4.0 build, including DES (which moved to the legacy
 * provider in 3.x). */
["md5", "apr1", "aixmd5", "sha256", "sha512", "crypt"].forEach(function(mode){
    testFeature("passwd " + mode + " round-trip", function(){
        var pw = crypto.passwd("mypasswd123", null, mode);
        if (!pw || !pw.line || pw.mode !== mode) return false;
        var comp = crypto.passwdComponents(pw.line);
        if (!comp || comp.mode !== mode || comp.line !== pw.line) return false;
        return crypto.passwdCheck(pw.line, "mypasswd123") === true
            && crypto.passwdCheck(pw.line, "wrongPassword") === false;
    });
});

/* Known-answer tests from Drepper's SHA-crypt.txt
 * (https://www.akkadia.org/drepper/sha-crypt.txt). */
testFeature("passwd sha256 Drepper KAT", function(){
    var pw = crypto.passwd("Hello world!", "saltstring", "sha256");
    return pw.line === "$5$saltstring$5B8vYYiY.CVt1RlTTf8KbXBH3hsxY/GNooZaBBGWEc5";
});

testFeature("passwd sha512 Drepper KAT", function(){
    var pw = crypto.passwd("Hello world!", "saltstring", "sha512");
    return pw.line === "$6$saltstring$svn8UoSVapNtMuq1ukKS4tPQd8iKwSMHWjl"
