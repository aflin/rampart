...
We would like to thank Ray Ageloff of the University of Rhode Island for
his “*Primer on SQL*”, published by Times Mirror/Mosby College
...
heavily upon it in our explanations of SQL, in a way which can be easily
...
implemented. Our goal with this manual has been to document both the
features that are currently working, as well as those features that are
planned. If a feature is shown as not currently implemented, or you
don’t see the capabilites you need, please call us to determine our
current development timetable.

Documentation Structure
-----------------------

There are three distinct sections to this manual. Where you start will
depend on your needs. Chapters [chp:Intro] to [chp:AdmDB] of the book
deal with the abilities of Texis and provide an introduction to SQL.
These chapters also note the differences between Texis and a typical SQL
database. These chapters can be skimmed by a person knowledgeable about
SQL to note the differences with standard databases. In particular you
may want to read about Metamorph queries in Chapter [Chp:MMLike], and
the section on ``LIKE`` and friends.

Part V, Chapter [Part:V:Chp:Embed] explains how to use the Texis server
from within a C program. This chapter should be read be anyone intending
to write a program to access Texis.

Part V, Chapter [Part:V:Chp:Prog] describes the programs that come with
Texis, including the daemon and ad-hoc query tool. This chapter is
divided into three sections. These are the example programs, which are
complete examples of how to program with Texis, the programs required to
use Texis, and finally some maintenance and administration tools that
are useful.
...
that may often be either large or small, or which Texis can manage more
...
Table [tab:RelDBTerm] lists these terms and shows their relationship.
...
Figure [fig:PatInvRel] illustrates two relations. The first one depicts
...
Figure [fig:PatInvRel], invoice 71115 in the INVOICE relation is related
...
Data Types:
    ...
    *indirect*. Table [tab:DTypes] illustrates the general format for
    ...
    UINT64:
        ...
        18,446,744,073,709,551,616. Added in version 6.
    ...
    BLOBZ:
        ...
        The BLOBZ type is only supported in Texis Version 8 and later.
    ...
    STRLST:
        ...
        undefined for strlst, though in Texis version 7 and later
...
Table [tab:CompOp] shows the comparison operators allowed in Texis.
...
#. The department is in the Corporate Division with a budget under
   $12,000.
...
#. The department is in either the Corporate Division or the Product
   Division.
...
Parentheses are optional but are included to improve readability of the
condition.
...
diagonally-opposite corners of the bounding box. [1]_ They are typically
also ``long`` geocode values. However, in version 5.01.1194651000
20071109 and later, they may each be a single ``varchar`` (text) value
...
values. In Texis version 7 and later it behaves similar to the SUBSET
operator (p. ), i.e. it is true if all left-side value(s) are also
present on the right-side. (See below for version 6 and earlier
differences.)
...
In Texis version 6 (or ``compatibilityversion`` 6) and earlier, ``IN``
behaved much like the ``INTERSECT`` operator (p. ) instead of
``SUBSET``, i.e. it was true if *any* left-side value was present on the
right-side. This behavior can be restored with the ``inmode`` SQL
property, p.  (or the ``compatibilityversion`` property, p. ). Note
however that with a single non-empty left-side value, there is no
difference, as intersection and subset then behave the same.

Additionally, ``IN`` with version 6 and earlier did not always utilize
indexes (e.g. if the table column was on the right-side), and had other
quirks.
...
INTERSECT share in common. SUBSET was added in Texis version 7.
...
-  | **Empty ``strlst`` column left-side, non-empty parameter
     right-side:**
...
behaviors in common in Texis version 7 (or ``compatibilityversion`` 7)
and later:
...
See also p.  for version 6 and earlier IN issues.
...
changed by an ``ORDER BY`` clause. Historically (prior to Texis version
8), an ``ORDER BY`` containing ``$rank`` (or potentially any expression
...
Thus, in Texis version 8 and later, ``ORDER BY`` clauses containing
...
in version 8 and later, most ORDER BY $rank clauses should probably be
ORDER BY $rank DESC, to get rank-descending behavior. This behavior may
be altered by the ``legacyversion7orderbyrank`` SQL setting (p. ).

For fuller details on using the logic operators in ``LIKE`` see
Chapter [Chp:MMLike]
...
.. [1]
   Prior to version 5.01.1194489000 20071107, the box had to be
   specified as “lower-left”, “upper-right” (i.e. SW, NE) only. In that
   version (and later), other diagonal combinations (i.e. NW, SE) are
   supported.
