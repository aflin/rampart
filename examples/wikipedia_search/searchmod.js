...
function search(req) {
                ...
                '<table style="background-color: white; width:100%">'+
                  '<tr>'+
                    '<td style="position:relative">'+
                      '<input autocomplete="off" type="text" id="fq" name="q" value="'+q+'" placeholder="Search" style="box-sizing:border-box;min-width:150px;width:100%;height:30px;font:normal 18px arial,sans-serif;padding: 1px 3px;border: 2px solid #ccc;">'+
    ...
    //this will push e.g. q="v8(javascript)" higher in the results when req.query.q="v8"
    //see https://docs.thunderstone.com/site/texisman/rank_knobs.html
    //and https://docs.thunderstone.com/site/texisman/other_ranking_properties.html
    //sql.exec("set likepleadbias=1000");
    // or sql.set({ "likepleadbias":1000 });
    ...
    // %U is for url encoding
    if (icount > 10+skip) {
        skip+=10
        html+=sprintf('</div><br><div style="text-align:right;padding-top: 12px;width: 450px;clear: both;"><a href="/search.html?q=%U&skip=%d">Next 10</a></div></body></html>',req.query.q,skip);
    } else {
        html+='</div></body></html>';
    }
