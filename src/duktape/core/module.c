...
static duk_ret_t load_so_module(duk_context *ctx)
{
  ...
  if (!lib)
  {
    ...
    if (!strstr(dl_err, "No such file") && !strstr(dl_err, "not found"))
    {
  ...
  duk_c_function init = (duk_c_function)dlsym(lib, "dukopen_module");
  pthread_mutex_unlock(&lock);
  if (init != NULL)
  {
...
static duk_ret_t require(duk_context *ctx)
{
  ...
  for (module_loader_idx = 0; module_loader_idx < sizeof(module_loaders); module_loader_idx++)
  {
  ...
  if (id == NULL)
  {
    duk_push_error_object(ctx, DUK_ERR_ERROR, "Could not resolve module id: %s\n", strerror(errno));
    return duk_throw(ctx);
  }
