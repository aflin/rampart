...
int debugl=rpydebug;
#define rp_debug_printf(level, ...) do{\
    if(level<=debugl){\
...
#define rp_debug_printf_pyvar(level, _s, _v, ...) do{\
    if(level<=debugl){\
        char *_s = pytostr(_v);\
        if(is_child) printf("(%d) at %d (fork   %d): ", (int)getpid(),__LINE__, is_child);\
        else         printf("(%d) at %d (thread %d): ", (int)getpid(),__LINE__, get_thread_num());\
...
#define rp_debug_printfhex(level,_h,_hsz,...) do{\
    if(level<=debugl){\
        if(is_child) printf("(%d) at %d (fork   %d): ", (int)getpid(),__LINE__, is_child);\
        else         printf("(%d) at %d (thread %d): ", (int)getpid(),__LINE__, get_thread_num());\
        printf(__VA_ARGS__);\
...
#define RP_Py_XDECREF(r) do{\
    if(r){\
        ...
        char *s=pytostr((r));\
        rp_debug_printf(5,"xdecref of %p, refcnt was %d for %s\n",(r),cnt, s);\
        free(s);\
        /* deallocated objects often return a random negative number*/\
        if(cnt<0) rp_debug_printf(1,"DANGER WILL ROBINSON!!! possible xdecref on deallocated python object\n");\
        if(cnt>0) Py_XDECREF((r));\
        if(!cnt) rp_debug_printf(1,"DANGER, attempted xdecref on an item with refcnt=0\n");\
        r=NULL;\
    }\
    /*else  rp_debug_printf(5,"xdecref, ref was null\n");*/\
} while(0)

#define PYUNLOCK(s) do {\
    if(PyErr_Occurred()){\
        rp_debug_printf(1,"Unhandled exception");\
        PyErr_Clear();\
    }\
    if(!is_child) {\
        PyGILState_Release((s));\
        rp_debug_printf(5,"RELEASED pylock\n");\
    }\
}while (0)
...
#define rp_debug_printf_pyvar(...) /* nada */

#define rp_debug_printfhex(...) /* nada */

#define dcheckvar(...) /* nada */

#define RP_Py_XDECREF(r) do{\
/*printf("xdecref %p (%d refs) on line %d\n",r, r?(int)Py_REFCNT((r)):-1, __LINE__);*/\
...
#define PYLOCK ({\
    ...
    if(!is_child) {\
        rp_debug_printf(5,"GETTING pylock\n");\
        /*if (!PyGILState_Check()) rp_debug_printf(4,"is already locked\n");*/\
        _s=PyGILState_Ensure();\
        rp_debug_printf(5,"GOT pylock\n");\
    }\
    _s;\
})
...
static int send_val(PFI* finfo, PyObject *pRef, char *err)
{
    ...
    else
    {
        ...
        rp_debug_printf(4,"extracting bytes from pBytes=%p\n", pBytes);
...
PyObject *receive_pval(PFI *finfo, char **err)
{
    ...
    rp_debug_printf(4, "got val from child, type %c\n",type);
    ...
    *err=NULL;
    ...
    else if (type=='s')
    {
        ...
        rp_debug_printf(4, "got string val from child, %s\n", str);
...
static void py_kill_child(void *arg)
{
    ...
    kill(*kpid,SIGTERM);
    rp_debug_printf(4,"killed child %d\n",(int)*kpid);
...
/* the first time a python function is accessed from a particular
   JS function or passed to a python function within JS,
   pref will be the parent object, and attr_fname will be the attribute name
   These are processed through parent_get() in order to get the
   actual function and attributes.

   Why? We cannot get attributes, then get the attributes of attributes,
   and then get those atributes, etc., potentially infinitum
   (if some are self referencing), so this is a lazy load of sorts.

   idx refers to location of current_function or 'this'
*/
static void parent_fix_pval(duk_context *ctx, duk_idx_t idx)
{
    if(duk_get_prop_string(ctx, idx, DUK_HIDDEN_SYMBOL("attr_fname")))
    {   // the first time a function is called, pvalue==parentvalue
        // we need to look up the actual pvalue in child process
        // and fill in the real pvalue, plus all attributes
        const char *key=duk_get_string(ctx, -1);
        char *err=NULL;
        PyObject *parentValue=NULL;

        duk_pop(ctx); //attr_name

        if(!duk_get_prop_string(ctx, idx, DUK_HIDDEN_SYMBOL("pref")))
            RP_THROW(ctx, "internal error getting python object pointer");
        parentValue=duk_get_pointer(ctx, -1);

        duk_pop(ctx);// parentValue pointer

        if(!parentValue)
            RP_THROW(ctx, "internal error getting python object pointer");

        err=parent_get(parentValue, key, -1, duk_normalize_index(ctx, idx) /* must be positive */ );
        ...
        duk_pop(ctx); //duplicate of current_function

        // do this only once:
        duk_del_prop_string(ctx, -1, DUK_HIDDEN_SYMBOL("attr_fname"));
        duk_push_object(ctx);

        //an object to be finalized and perform decref in child
        duk_get_prop_string(ctx, -2, DUK_HIDDEN_SYMBOL("pref"));
        duk_put_prop_string(ctx, -2, DUK_HIDDEN_SYMBOL("pref"));
        duk_put_prop_string(ctx, -2, DUK_HIDDEN_SYMBOL("pref_obj"));
        duk_push_c_function(ctx, pvalue_finalizer, 1);
        duk_set_finalizer(ctx, -2);
    }
    else
        duk_pop(ctx); // undefined attr_name
}
...
static void parent_finalizer(PyObject *pModule)
{
    ...
    rp_debug_printf(4,"Finalized in parent for %p\n", pModule);
    ...
    rp_debug_printf(4,"CONFIRMED Finalized in parent for %p\n", pModule);
...
static int child_finalizer(PFI *finfo)
{
    ...
    rp_debug_printf(4,"Finalized in child for %p\n", pModule);
...
static duk_ret_t _get_pref_val(duk_context *ctx)
{
    ...
    if(type=='m')
    {
        ...
        rp_debug_printf_pyvar(4, x, pValue, "got pickle val from child, %s\n", x);
        RP_Py_XDECREF(pValue);
        PYUNLOCK(state);
...
static void put_func_attributes(duk_context *ctx, PyObject *pValue, PyObject *pRef, const char *attr_fname, const char *pstring)
{
    ...
    // hidden pyvar pointer as property of function
    if(pValue)
    {
    ...
    // if set, pValue or pRef refer to the parent object.  Do lazy lookup later
    if(attr_fname)
    {
        duk_push_string(ctx, attr_fname);
        duk_put_prop_string(ctx, -2, DUK_HIDDEN_SYMBOL("attr_fname"));
    }
...
static inline void make_pyval(duk_context *ctx, PyObject *pValue, duk_c_function strfunc, duk_c_function valfunc, const char*fn, PyObject *pRef, char *refstr)
{
    ...
    if(refstr){
        rp_debug_printf(4,"putting refstr %s on strfunc\n", (char*)refstr);
...
static PyObject *parent_import(duk_context *ctx, const char *script, int typeno, char **fnames, char **fstring)
{
    ...
    rp_debug_printf(4,"in parent got pmod=%p\n",pModule);
...
static int child_import(PFI *finfo, int type)
{
    ...
    rp_debug_printf(4,"in child_import\n");
    if (forkread(&script_sz, sizeof(size_t))  == -1)
    {
        rp_debug_printf(4,"fail read script_sz");
        return 0;
    }
    ...
    {
        rp_debug_printf(4,"fail read script");
        return 0;
    }
    ...
    rp_debug_printf(4,"in child_import - python_is_init=%d\n",python_is_init);
    if(!python_is_init)
    ...
    if(type)
    {
        ...
        rp_debug_printf(4,"in _child_import - pCode = %p\n", pCode);

        if(!pCode)
        {
            exc = get_exception(buf);
            rp_debug_printf(4,"child_import: !pCode: %s, script:%s\n%s\n",exc, scriptname, script);
        }
        else
        {
    ...
    if(!exc)
    {
        ...
        if(pResStr)
            funcstring = PyUnicode_AsUTF8(pResStr);
        else
            funcstring = "(unknown pyfunction)";
...
int child_get_val(PFI* finfo)
{
    ...
    rp_debug_printf(4,"translating pRef=%p to pickle bytes\n", pRef);
    pBytes=PyPickle_WriteObjectToString(pRef,Py_MARSHAL_VERSION);
...
static char *parent_read_val(PFI *finfo, duk_idx_t existing_func_idx)
{
    char retval='X';
    char *errmsg=NULL;
    ...
    else if(retval == 'r') // returned a ref, get reference to var, var stays in child
    {
        ...
        rp_debug_printf(4,"receiving refstring, sz=%d\n", (int)sz);

        if(forkread(refstr, sz) == -1)
            return strdup(pipe_error);

        rp_debug_printf(4,"refstring=%s\n", refstr);
        rp_debug_printf(4,"STORING pRef=%p in JS object\n", pRef);
        ...
        // do this before make_pyval, which might trigger finalizers
        // and disrupt the flow of info over the pipe
        errmsg=parent_py_call_read_error(finfo);
        if(!errmsg)
            return strdup(pipe_error);
        if(!pRef)
        ...
        {
            //when called from parent_named_call
            if(existing_func_idx > -1)
                duk_dup(ctx, existing_func_idx);
            else
        ...
        {
            rp_debug_printf(5, "make_pyval\n");
            make_pyval(ctx, NULL, _get_pref_str, _get_pref_val, funcnames, pRef, refstr);
        }
    ...
    {
        rp_debug_printf(4,"in parent_read_val, got bad response '%c' 0x%2x\n", retval, retval);
...
static char *parent_py_call(PyObject * pModule, const char *fname)
{
    ...
    rp_debug_printf(4,"parent_py_call, top=%d\n", (int)top);
            ...
            if( duk_is_object(ctx, i) && duk_has_prop_string(ctx, i, "pyArgs") )
            {
                ...
                while (duk_next(ctx,-1,1))
                {
                    ...
                    )
                    {
                        ...
                        rp_debug_printf(4,"PUT pRef from storage in PyObj, sending pRef=%p\n", refptr);
                        npPtrs++;
                    }
    ...
    if(pickle_sz)
    {
        rp_debug_printf(4,"in parent_py_call - sending: pickle=%p, pickle_sz=%d\n",pickle, (int)pickle_sz);
    ...
    return parent_read_val(finfo, -1);
}
...
static PyObject *py_call_in_child(char *fname, PyObject *pModule, PyObject *pArgs, PyObject *kwdict, char **errmsg)
{
    ...
    rp_debug_printf_pyvar(4,x,pModule,"got func %s(%p) from %s, function callable = %d\n", fname, pFunc, x, pFunc?(int)PyCallable_Check(pFunc):0);
    ...
    rp_debug_printf(4,"CALLING with args=%p, nargs=%d\n", pArgs, (int) (pArgs ? PyTuple_Size(pArgs) : 0) );

    dcheckvar(4,pArgs);
    dcheckvar(4,pFunc);
    rp_debug_printf_pyvar(4, x, pFunc, "tpcall func=%s\n", x );
    rp_debug_printf_pyvar(4,x,pArgs, "tpcall args=%s\n", x );
    ...
    rp_debug_printf_pyvar(4, x, pValue, "returning value %p %s\n", pValue, x );
    return pValue;
}
...
static int child_py_call_write_error(PFI *finfo, char *errmsg)
{
    ...
    rp_debug_printf(4,"writing error msg: '%s'\n", errmsg);
    sz=strlen(errmsg)+1;
...
static int child_write_var(PFI *finfo, PyObject *pRes, char *errmsg)
{
    ...
    else
    {
        ...
        rp_debug_printf(4,"funcs = %s\n", funcnames);

        if(forkwrite("r", sizeof(char)) == -1)
            return 0;

        rp_debug_printf(4,"SENDING pRes=%p for storage\n", pRes);
        ...
        //rp_debug_printf(4,"sending %s, size=%d\n", resstr, (int)str_sz);
...
static int child_py_call(PFI *finfo)
{
    ...
    rp_debug_printf(4,"in child_py_call, pmod=%p\n", pModule);
    ...
    if(pickle_sz > 0)
    {
        ...
        if(!pArgs || !PyTuple_Check(pArgs) )
        {
            ...
            rp_debug_printf(4,"writing error %s", errmsg);
...
static char *parent_get(PyObject *parentVal, const char *key, int index, duk_idx_t existing_func_idx)
{
    ...
    rp_debug_printf(4, "parent_get, parentVal=%p, key=%s, idx=%d\n", parentVal, key, (int)index);

    if(index>-1) //accessing list/tuple index
    {
        if(forkwrite("G", sizeof(char)) == -1)
            return strdup(pipe_error);
    }
    else  //accessing attribute of object
    {
        if(forkwrite("g", sizeof(char)) == -1)
            return strdup(pipe_error);
    }
    ...
    if(index>-1)
    {
        if(forkwrite(&index, sizeof(int)) == -1)
            return strdup(pipe_error);
    }
    else
    {
    ...
    return parent_read_val(finfo, existing_func_idx);
}

static int child_get(PFI *finfo, int using_index)
{
...
static void do_fork_loop(PFI *finfo)
{
    while(1)
    {
        ...
        if(PyErr_Occurred())
        {
            char buf[MAX_EXCEPTION_LENGTH];
            char *exc = get_exception(buf);
            rp_debug_printf(1, "EXCEPTION: '%c' %s\n", command, &exc[1]);
            exit(1);
        }
#endif
...
static PFI *check_fork()
{
    ...
    PFI *finfo = NULL;
    rp_debug_printf(5,"check_fork(), threadno=%d\n", threadno);
    ...
    if (!finfo->childpid || waitpid(finfo->childpid, &pidstatus, WNOHANG))
    {
        if (is_forked)
        {
            rp_debug_printf(4,"ERROR:  we should have child process %d, but appears to be dead\n", finfo->childpid);
            finfo->childpid=0;
            finfo->reader=-1;
            finfo->writer=-1;
...
#define get_pval(idx, needlock) ({\
    duk_idx_t _idx = duk_normalize_index(ctx, idx);\
    rp_debug_printf(4, "starting get_pval, idx=%d idx=%d\n", (int)_idx, (int)idx);\
    PyObject *_pval=NULL;\
    if( duk_get_prop_string(ctx, _idx, DUK_HIDDEN_SYMBOL("pvalue")) ) {\
        _pval=duk_get_pointer(ctx, -1);\
    };\
    duk_pop(ctx);\
    /* the lazy load if attr_fname is present */\
    rp_debug_printf(4,"checking for attr_fname\n");\
    if( _pval && duk_get_prop_string(ctx, _idx, DUK_HIDDEN_SYMBOL("attr_fname")) ) {\
        PyGILState_STATE st;\
        const char *attr_fname = duk_get_string(ctx, -1);\
        rp_debug_printf(4,"attr = '%s'\n",attr_fname);\
        PyObject *p=NULL;\
        if(needlock) st = PYLOCK;\
        p = PyObject_GetAttrString(_pval, attr_fname);\
        /* REPLACE pvalue with the value found under attr_fname */\
        duk_push_pointer(ctx, p);\
        duk_put_prop_string(ctx, _idx, DUK_HIDDEN_SYMBOL("pvalue"));\
        /* REMOVE IT since we now have the proper pvalue */\
        duk_del_prop_string(ctx, _idx, DUK_HIDDEN_SYMBOL("attr_fname"));\
...
static duk_ret_t _p_to_value(duk_context *ctx)
{
    ...
    if(pValue)
    {
        start_pytojs(ctx);
        state = PYLOCK;
        rp_debug_printf_pyvar(4,x,pValue,"p_to_val for %s\n", x);
        push_ptype(ctx, pValue);
        PYUNLOCK(state);
    }
    else
...
static duk_ret_t pvalue_finalizer(duk_context *ctx)
{
    ...
    if(pValue)
    {
        ...
        rp_debug_printf_pyvar(5,x,pValue,"decref for %p (%s) with refcnt = %d\n", pValue, x, (int)Py_REFCNT(pValue));
        RP_Py_XDECREF(pValue);
        PYUNLOCK(state);
...
static void get_pyval_and_push(duk_context *ctx, duk_idx_t idx, const char *key, int index)
{
    ...
    if(!duk_get_prop_string(ctx, idx, DUK_HIDDEN_SYMBOL("pvalue")))
    {
        ...
        if(!parentValue)
        {
            rp_debug_printf(4,"looking for %s in NULL, returning undefined\n", key);
            duk_push_undefined(ctx);
            return;
        }
        rp_debug_printf(4,"looking for %s in child\n", key);
        err=parent_get(parentValue, key, index, -1);
...
static duk_ret_t _proxyget(duk_context *ctx)
{
    ...
    if(!key)
    {
        index = duk_get_int_default(ctx, 1, -1);
        if(index == -1)
            return 0;
...
static duk_ret_t _py_call(duk_context *ctx, int is_method)
{
    ...
    if(!is_child && must_fork)
    {
        ...
        duk_pop_2(ctx);
        rp_debug_printf(4,"in py_call, pmod=%p, ctx=%p, fname=%s\n", pModule, ctx, fname);
    ...
    rp_debug_printf(4,"Got function/method, pmod=%p, ctx=%p\n", pFunc, ctx);

    if (!pFunc || !PyCallable_Check(pFunc))
    {
        rp_debug_printf(4,"ERROR in py_call, pmod=%p, ctx=%p\n", pFunc, ctx);
        err="error calling python function: %s";
        goto end;
    }
    ...
    // this callable pFunc was retrieved by key (fname) above in PyObject_GetAttrString
    // so we will let it go here too.
    if(is_method)
        RP_Py_XDECREF(pFunc);
...
static void put_callPyFunc(duk_context *ctx, PyObject *pModule)
{
    ...
    if(!is_child && must_fork)
        put_func_attributes(ctx, NULL, pModule, NULL, NULL);
    else
