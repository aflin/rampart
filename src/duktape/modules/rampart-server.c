...
/* malloc, clone and steal aux buffer */
static DHS *clone_dhs(DHS *indhs)
{
    DHS *dhs = NULL;

    REMALLOC(dhs, sizeof(DHS));
    memcpy (dhs, indhs, sizeof(DHS));
    /*
...
static int push_req_vars(DHS *dhs)
{
    ...
    */
    ...
    if(dhs->aux)
    {
        if( strlen(dhs->aux) > 2 && !strncmp(dhs->aux,"500",3))
        {
...
/*
    Send a file, whole or partial with range if requested
      ...
      sendfile_flags - bitmask: SENDFILE_NO_RANGE_CAP, SENDFILE_NO_ACCEPT_RANGES
*/
...
static void rp_sendfile(evhtp_request_t *req, char *fn, int haveCT, struct stat *filestat, int is_fileserver, uint16_t sendfile_flags)
{
    ...
    if (range && !strncasecmp("bytes=", range, 6))
    {
        ...
        if (eptr != range + 6)
        {
            ...
            else if (!(sendfile_flags & SENDFILE_NO_RANGE_CAP) && filesize - beg > (ev_off_t)default_range_bytes)
...
static evhtp_res obj_to_buffer(DHS *dhs)
{
    ...
    uint16_t sendfile_flags = 0;
    if (duk_get_prop_string(ctx, -1, "noRangeCap"))
    {
        if (duk_is_boolean(ctx, -1) && duk_get_boolean(ctx, -1))
            sendfile_flags |= SENDFILE_NO_RANGE_CAP;
    }
    duk_pop(ctx);
    if (duk_get_prop_string(ctx, -1, "noAcceptRanges"))
    {
        if (duk_is_boolean(ctx, -1) && duk_get_boolean(ctx, -1))
            sendfile_flags |= SENDFILE_NO_ACCEPT_RANGES;
    }
    duk_pop(ctx);
    ...
    while (duk_next(ctx, -1, 1))
    {
        ...
        if (mres)
        {
            ...
            if (duk_is_string(ctx, -1) && ((d = duk_get_string(ctx, -1)) || 1) && *d == '@')
            {
                ...
                {
                    rp_sendfile(dhs->req, (char *)d+1, 1, NULL, 0, sendfile_flags);
                    return (0);
                }
            }
...
static void http_callback(evhtp_request_t *req, void *arg)
{
    ...
    newdhs.auxbuf= NULL;
    newdhs.freeme=0;
    newdhs.skip_wrap=dhs->skip_wrap;
    dhs = &newdhs;
...
/* Set when fd 1/2 were pointed at the logs; rotation must redo the dup2
 * or the descriptor keeps writing to the rotated-away file. */
...
/* Detach the daemon's std streams.  Runs in the grandchild right after
 * the double fork: held any longer, a slow-starting daemon is still on
 * the launcher's terminal when sudo (use_pty) tears it down, and dies.
 * Needs logging() to have run -- it opens access_fh/error_fh. */
static void daemonize_stdio(void)
{
    ...
    /* fd 1/2 follow the logs, not just the FILE *s: writes straight to
     * the descriptor (ggml, abort, a child) would otherwise be lost. */
    ...
    stdout = access_fh;
    stderr = error_fh;
    /* line buffered: FILE * and descriptor share a file now */
    setvbuf(access_fh, NULL, _IOLBF, 0);
    setvbuf(error_fh,  NULL, _IOLBF, 0);
}
    ...
    */
    ...
    if (daemon)
    {
        ...
        {   /* child */
            ...
            /* release the launcher's terminal before the slow part */
            daemonize_stdio();
...
// Use at your own risk.
    ...
    */
    ...
    if (daemon)
    {
        /* std streams were detached in daemonize_stdio(), after the fork */
        duk_pull(ctx,0);
