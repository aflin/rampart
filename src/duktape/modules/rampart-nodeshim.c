...
"    function done() { if (ownsFd) try { closeSync(fd); } catch(_){} }\n"
...
"  }\n"
"\n"
"  var readFile      = _makeAsync(readFileSync);\n"
"  var writeFile     = _makeAsync(writeFileSync);\n"
"  var appendFile    = _makeAsync(appendFileSync);\n"
...
"    } catch (e) {}\n"
"    _promisesCache = {\n"
"      readFile:   _makePromise(readFileSync),\n"
"      writeFile:  _makePromise(writeFileSync),\n"
"      appendFile: _makePromise(appendFileSync),\n"
...
"    'hmac':'HMAC',\n"
"    'aes-cbc':'AES-CBC', 'aes-ctr':'AES-CTR', 'aes-gcm':'AES-GCM', 'aes-kw':'AES-KW',\n"
...
"    var canon = _algCanonical[alg.name.toLowerCase()];\n"
"    if (!canon) throw _notSupported('Algorithm not supported: ' + alg.name);\n"
"    /* Return a shallow copy with the canonical name. */\n"
"    var out = {name: canon};\n"
"    for (var k in alg) if (k !== 'name' && Object.prototype.hasOwnProperty.call(alg, k)) out[k] = alg[k];\n"
"    return out;\n"
"  }\n"
...
"      throw _opErr(alg.name + ': namedCurve must be one of P-256, P-384, P-521');\n"
...
"      isPrivate = true;\n"
"    } else if (format === 'raw') {\n"
"      /* Web Crypto: 'raw' is public only — uncompressed point 04||X||Y. */\n"
...
"      pem = isPrivate ? _ecPkcs8FromXYD(curve, x, y, _b64uDec(keyData.d))\n"
"                      : _ecSpkiFromXY(curve, x, y);\n"
"    } else {\n"
...
"  var _okpNames = {'Ed25519':1, 'X25519':1};\n"
"  var _ED25519_OID  = _derOid([1, 3, 101, 112]);\n"
"  var _X25519_OID   = _derOid([1, 3, 101, 110]);\n"
"  var _ED25519_ALG  = _derSeq([_ED25519_OID]);\n"
"  var _X25519_ALG   = _derSeq([_X25519_OID]);\n"
"  function _okpAlgId(curve) {\n"
"    return (curve === 'Ed25519') ? _ED25519_ALG : _X25519_ALG;\n"
"  }\n"
"  function _okpAllowedUsages(name, isPrivate) {\n"
"    if (name === 'Ed25519') return isPrivate ? ['sign'] : ['verify'];\n"
"    return isPrivate ? ['deriveBits', 'deriveKey'] : [];\n"
"  }\n"
"  function _okpSpkiFromX(curve, x) {\n"
"    return _derToPem('PUBLIC KEY',\n"
"      _derSeq([_okpAlgId(curve), _derBitString(_padLeft(x, 32))]));\n"
"  }\n"
"  function _okpPkcs8FromD(curve, d) {\n"
"    /* PrivateKeyInfo's privateKey field is OCTET STRING containing\n"
"       another OCTET STRING with the 32-byte scalar. */\n"
"    var inner = _derOctetString(_padLeft(d, 32));\n"
"    var outer = _derSeq([\n"
"      _derInteger(Buffer.from([0])),\n"
"      _okpAlgId(curve),\n"
...
"    var mat = _unbox(key);\n"
"    var fn = (key.algorithm.name === 'Ed25519') ? rc.ed25519_components : rc.x25519_components;\n"
"    var comps = fn(mat.pem);\n"
...
"      isPrivate = true;\n"
"    } else if (format === 'raw') {\n"
"      /* Web Crypto OKP raw = 32-byte public key. */\n"
"      var raw = _toBuf(keyData);\n"
"      if (raw.length !== 32) throw _dataErr(alg.name + ' raw: expected 32 bytes');\n"
...
"      isPrivate = (keyData.d != null);\n"
"      pem = isPrivate ? _okpPkcs8FromD(alg.name, _b64uDec(keyData.d))\n"
"                      : _okpSpkiFromX(alg.name, _b64uDec(keyData.x));\n"
"    } else {\n"
...
"          throw _invalidAccess('Ed25519: key algorithm mismatch');\n"
"        if (key.type !== 'private') throw _invalidAccess('sign: key.type must be private');\n"
"        var edSig = rc.ed25519_sign(_toBuf(data), mat.pem);\n"
...
"          throw _invalidAccess('Ed25519: key algorithm mismatch');\n"
"        if (key.type !== 'public') throw _invalidAccess('verify: key.type must be public');\n"
"        var edOk = rc.ed25519_verify(_toBuf(data), mat.pem, _toBuf(signature));\n"
...
"  var _aesNames = {'AES-CBC':1, 'AES-CTR':1, 'AES-GCM':1, 'AES-KW':1};\n"
"  function _aesCipher(algName, lenBits) {\n"
"    if (lenBits !== 128 && lenBits !== 192 && lenBits !== 256)\n"
"      throw _opErr('AES: length must be 128, 192, or 256');\n"
"    var mode = algName.substring(4).toLowerCase();  /* 'cbc'|'ctr'|'gcm'|'kw' */\n"
"    if (mode === 'kw') mode = 'wrap';\n"
"    return 'aes-' + lenBits + '-' + mode;\n"
"  }\n"
"  /* JWK alg field for AES.  RFC 7518: A128GCM/A192GCM/A256GCM,\n"
"     A128KW/A192KW/A256KW.  AES-CBC and AES-CTR don't have a\n"
"     standard JWK alg name when used alone (CBC composites with HMAC\n"
"     are JOSE-only); leave .alg unset in those cases. */\n"
...
"    var alg = _normAlg(algorithm);\n"
"    if (!_aesNames[alg.name]) throw _typeErr('not an AES algorithm');\n"
"    var bytes;\n"
"    if (format === 'raw') {\n"
"      bytes = _toBuf(keyData);\n"
"    } else if (format === 'jwk') {\n"
"      if (!keyData || keyData.kty !== 'oct')\n"
"        throw _dataErr('AES JWK: kty must be \"oct\"');\n"
"      bytes = _b64uDec(keyData.k);\n"
"      var expectedAlg = _aesJwkAlg(alg.name, bytes.length * 8);\n"
"      if (expectedAlg && keyData.alg && keyData.alg !== expectedAlg)\n"
"        throw _dataErr('AES JWK: alg mismatch — expected ' + expectedAlg + ', got ' + keyData.alg);\n"
"    } else {\n"
"      throw _notSupported('AES importKey: format must be raw or jwk');\n"
"    }\n"
...
"  function _aesCrypt(direction, algorithm, key, data) {\n"
"    var alg = _normAlg(algorithm);\n"
"    if (key.algorithm.name !== alg.name)\n"
...
"      if (tagBits % 8 !== 0 || tagBits < 32 || tagBits > 128)\n"
"        throw _opErr('AES-GCM: tagLength must be 32/64/96/104/112/120/128');\n"
...
"    }\n"
"    var out = (direction > 0) ? rc.encrypt(opts) : rc.decrypt(opts);\n"
"    return _toAB(out);\n"
"  }\n"
...
"    if (format !== 'raw') throw _notSupported(alg.name + ' importKey: format must be raw');\n"
...
"        var shared = rc.ecdh(mat.pem, pubMat.pem);\n"
"        var sBuf = Buffer.isBuffer(shared) ? shared : Buffer.from(shared);\n"
"        if (typeof length === 'number') {\n"
"          if (length % 8 !== 0) throw _opErr('deriveBits: length must be a multiple of 8');\n"
"          if (length >> 3 > sBuf.length) throw _opErr('deriveBits: requested length exceeds ECDH output');\n"
"          sBuf = sBuf.slice(0, length >> 3);\n"
"        }\n"
"        return _toAB(sBuf);\n"
"      }\n"
"      if (alg.name === 'PBKDF2') {\n"
...
"          throw _opErr('PBKDF2: length must be a positive multiple of 8 bits');\n"
...
"          throw _invalidAccess('X25519: baseKey is not an X25519 key');\n"
"        if (!alg.public || !(alg.public instanceof CryptoKey))\n"
"          throw _typeErr('X25519: algorithm.public must be a CryptoKey');\n"
"        if (alg.public.algorithm.name !== 'X25519')\n"
"          throw _invalidAccess('X25519: public key must be an X25519 key');\n"
"        var xPub = _unbox(alg.public);\n"
"        var xs = rc.x25519_derive(mat.pem, xPub.pem);\n"
"        var xBuf = Buffer.isBuffer(xs) ? xs : Buffer.from(xs);\n"
"        if (typeof length === 'number') {\n"
"          if (length % 8 !== 0) throw _opErr('X25519: length must be a multiple of 8');\n"
"          if (length >> 3 > xBuf.length) throw _opErr('X25519: requested length exceeds output');\n"
"          xBuf = xBuf.slice(0, length >> 3);\n"
"        }\n"
"        return _toAB(xBuf);\n"
"      }\n"
"      if (alg.name === 'HKDF') {\n"
...
"        if (alg.salt == null) throw _opErr('HKDF: salt required');\n"
"        if (alg.info == null) throw _opErr('HKDF: info required');\n"
...
"        var mat = _unbox(key);\n"
"        var ct  = rc.rsa_pub_encrypt(_toBuf(data), mat.pem, _rsaCryptOpts(alg, key));\n"
...
"        var mat = _unbox(key);\n"
"        var pt  = rc.rsa_priv_decrypt(_toBuf(data), mat.pem, _rsaCryptOpts(alg, key));\n"
...
"        var okpPair = (alg.name === 'Ed25519')\n"
"          ? rc.ed25519_gen_key() : rc.x25519_gen_key();\n"
...
"        if (!alg.namedCurve || !_ecCurveBytes[alg.namedCurve])\n"
"          throw _opErr(alg.name + ': namedCurve must be P-256, P-384, or P-521');\n"
...
"        }\n"
"        if (_aesNames[alg.name] || alg.name === 'RSA-OAEP') {\n"
"          /* Delegate to encrypt() — both paths support raw byte input. */\n"
"          return self.encrypt(wrapAlgorithm, wrappingKey, bytes);\n"
"        }\n"
...
"      } else if (_aesNames[alg.name] || alg.name === 'RSA-OAEP') {\n"
"        bytesPromise = self.decrypt(unwrapAlgorithm, unwrappingKey, wBuf).then(function(ab){ return Buffer.from(ab); });\n"
"      } else {\n"
...
"            key_ops: key.usages.slice()\n"
"          };\n"
"          var algField = _aesJwkAlg(algName, key.algorithm.length);\n"
"          if (algField) jwk.alg = algField;\n"
...
"          var fn = (algName === 'Ed25519') ? rc.ed25519_components : rc.x25519_components;\n"
"          return Promise.resolve(_toAB(_hexToBuf(fn(mat.pem).public)));\n"
"        }\n"
"        if (format === 'spki' || format === 'pkcs8') {\n"
...
static void nodeshim_init_crypto(duk_context *ctx)
{
    /* Build the classic crypto module first. */
    duk_push_string(ctx, "rampart-nodeshim.c:crypto_js");
    duk_compile_string_filename(ctx, DUK_COMPILE_FUNCTION, crypto_js);
    duk_call(ctx, 0);

    /* Build the Web Crypto wrapper and attach it as `.webcrypto` on
       the classic crypto module.  `require('crypto').webcrypto` then
       returns the Crypto singleton, matching node's layout. */
    duk_push_string(ctx, "rampart-nodeshim.c:webcrypto_js");
    duk_compile_string_filename(ctx, DUK_COMPILE_FUNCTION, webcrypto_js);
    duk_call(ctx, 0);
    /* Stack: [classic_crypto, webcrypto_exports].  Pluck the
       singleton out and attach. */
    duk_get_prop_string(ctx, -1, "crypto");
    duk_put_prop_string(ctx, -3, "webcrypto");
    duk_pop(ctx);  /* webcrypto_exports (still has Crypto/SubtleCrypto/CryptoKey
                     constructors — discarded; they live on the instance) */
}
/* ============================================================
...
"  'use strict';\n"
"\n"
"  /* ----- rampart-url, embedded ----- */\n"
"  function _qToO(qs) {\n"
"    var pairs = qs.split('&');\n"
"    var comp = {};\n"
"    var j = 0;\n"
"    if (typeof rampart !== 'undefined' && rampart && rampart.utils && rampart.utils.queryToObject) {\n"
"      comp = rampart.utils.queryToObject(qs);\n"
"      if (Object.keys(comp).length != pairs.length) {\n"
"        for (var i = 0; i < pairs.length; i++) {\n"
"          var pair = pairs[i];\n"
"          var pos = pair.indexOf('=');\n"
"          if (pos == -1) comp[j++] = decodeURIComponent(pair);\n"
"        }\n"
"      }\n"
"      return comp;\n"
"    }\n"
...
"        if (comp[key]) {\n"
"          if (typeof comp[key] == 'object') comp[key].push(val);\n"
"          else comp[key] = [comp[key], val];\n"
"        } else comp[key] = val;\n"
"      } else {\n"
...
"    return { raw: qs, components: _qToO(qs) };\n"
"  }\n"
...
"      var part = parts[i];\n"
"      if (part == '..') {\n"
"        if (outparts.length > 1) outparts.pop();\n"
"      } else if (part != '.') {\n"
"        outparts.push(part);\n"
"      }\n"
"    }\n"
...
"  }\n"
"  var portmap = {\n"
"    http: 80, https: 443, ftp: 21, ssh: 22, smpt: 25,\n"
"    ws: 80, wws: 443, tftp: 69, gopher: 70, sftp: 115,\n"
"    imap: 143, imaps: 933, pop3: 110, pop3s: 995, ftps: 990, telnets: 992\n"
"  };\n"
"  function components(url, pathOnly) {\n"
"    var ret = {\n"
"      scheme: '', username: '', password: '', origin: '',\n"
"      host: '', authority: '', path: '', fullPath: '',\n"
"      queryString: {}, hash: '', url: '', href: ''\n"
"    };\n"
"    var tmp = url, mtmp, pos;\n"
"    if (typeof url != 'string') return;\n"
"    if (!pathOnly) {\n"
"      var schemeNSlashes = 0, schemeSlashes = '', userPass = '';\n"
"      pos = tmp.indexOf(':');\n"
"      if (pos > 0) {\n"
"        ret.scheme = url.substring(0, pos).toLowerCase();\n"
"        if (!/^[a-z][a-z0-9\\+\\-\\.]+/.test(ret.scheme)) return;\n"
...
"      }\n"
"      if (tmp.charAt(0) == '/') {\n"
"        if (tmp.charAt(1) == '/') { schemeNSlashes = 2; schemeSlashes = '//'; }\n"
"        else                       { schemeNSlashes = 1; schemeSlashes = '/'; }\n"
"      }\n"
"      if (schemeNSlashes) tmp = tmp.substring(schemeNSlashes);\n"
"      mtmp = tmp.match(/^[^\\/\\?@]+@/);\n"
"      if (mtmp && mtmp.length) {\n"
"        userPass = mtmp[0];\n"
...
"      }\n"
"      mtmp = tmp.match(/^[a-zA-Z0-9\\-\\.\\_]*/);\n"
"      if (mtmp && mtmp.length && mtmp[0].length) {\n"
"        ret.host = mtmp[0].toLowerCase();\n"
"      } else {\n"
"        mtmp = tmp.match(/^\\[[0-9A-Fa-f:\\/]+\\]/);\n"
"        if (mtmp && mtmp.length) ret.host = mtmp[0].toLowerCase();\n"
"      }\n"
"      ret.authority = schemeSlashes + userPass + ret.host;\n"
"      /* origin per WHATWG never includes userinfo. Build it from host alone. */\n"
"      var originBase = ret.scheme + ':' + schemeSlashes + ret.host;\n"
...
"          if (ret.port != portmap[ret.scheme]) ret.portText = mtmp[0];\n"
"        } else return;\n"
"        tmp = tmp.substring(mtmp[0].length);\n"
"        ret.origin = originBase + (ret.portText ? ':' + ret.portText : '');\n"
"        ret.authority = ret.authority + (ret.portText ? ':' + ret.portText : '');\n"
"      } else {\n"
"        ret.port = portmap[ret.scheme];\n"
"        ret.origin = originBase;\n"
"      }\n"
"    } else tmp = url;\n"
"    pos = tmp.indexOf('#');\n"
"    if (pos > -1) { ret.hash = tmp.substring(pos); tmp = tmp.substring(0, pos); }\n"
"    var fchar = tmp.charAt(0);\n"
"    if (fchar != '/' && fchar != '' && fchar != '?') return;\n"
"    if (fchar == '') { ret.file = ''; ret.path = '/'; }\n"
"    else if (fchar == '?') {\n"
"      ret.path = '/'; ret.file = '';\n"
...
"    if (fchar == '/') ret.path = normalizePath(ret.path);\n"
"    ret.fullPath = ret.path + ret.file;\n"
"    if (pathOnly) return ret;\n"
...
"    if (/^javascript:/i.test(path)) return 'javascript:' + path.substring(11);\n"
"    var urlComp = components(path);\n"
"    if (urlComp) return includeComponents ? urlComp : urlComp.url;\n"
"    if (path.charAt(0) != '/') path = srcComp.path + path;\n"
"    urlComp = components(path, true);\n"
"    var ret = srcComp.origin + urlComp.fullPath +\n"
"              (urlComp.queryString.raw ? '?' + urlComp.queryString.raw : '');\n"
"    if (includeComponents) return components(ret);\n"
"    return ret;\n"
"  }\n"
...
"        ret.push(rewriteUrl(srcComp, urls[i], includeComponents));\n"
"      }\n"
"      return ret;\n"
...
"    if (!c) {\n"
"      /* pathname-only URL */\n"
"      var p = components(urlStr, true);\n"
"      if (p) {\n"
"        u.pathname = p.fullPath || null;\n"
"        u.hash = p.hash || null;\n"
"        if (p.queryString && p.queryString.raw !== undefined) {\n"
"          u.search = '?' + p.queryString.raw;\n"
"          u.query = parseQueryString ? p.queryString.components : p.queryString.raw;\n"
"          u.path = (u.pathname || '') + u.search;\n"
"        } else {\n"
"          u.query = parseQueryString ? {} : null;\n"
"          u.path = u.pathname;\n"
"        }\n"
"      }\n"
"      return u;\n"
"    }\n"
"    u.protocol = c.scheme ? c.scheme + ':' : null;\n"
"    u.slashes = (c.authority && c.authority.indexOf('//') === 0);\n"
"    if (c.username) {\n"
"      u.auth = c.username + (c.password ? ':' + c.password : '');\n"
"    }\n"
"    u.hostname = c.host || null;\n"
"    u.port = c.portText || (c.port != null && c.port !== portmap[c.scheme] ? String(c.port) : null);\n"
"    u.host = c.host ? (c.host + (u.port ? ':' + u.port : '')) : null;\n"
"    u.hash = c.hash || null;\n"
"    u.pathname = c.fullPath || null;\n"
"    if (c.queryString && c.queryString.raw !== undefined) {\n"
"      u.search = '?' + c.queryString.raw;\n"
"      u.query = parseQueryString ? c.queryString.components : c.queryString.raw;\n"
"      u.path = (u.pathname || '') + u.search;\n"
"    } else {\n"
"      u.query = parseQueryString ? {} : null;\n"
"      u.path = u.pathname;\n"
...
"  function format(u) {\n"
"    if (typeof u === 'string') {\n"
"      try { u = new URLClass(u); } catch (e) { return u; }\n"
"    }\n"
...
"  }\n"
"\n"
"  /* Route through the vendored punycode module for actual IDN conversion. */\n"
"  function domainToASCII(d) {\n"
"    if (d == null) return '';\n"
"    try { return require('punycode').toASCII(String(d)); }\n"
"    catch (e) { return String(d); }\n"
"  }\n"
"  function domainToUnicode(d) {\n"
"    if (d == null) return '';\n"
"    try { return require('punycode').toUnicode(String(d)); }\n"
"    catch (e) { return String(d); }\n"
"  }\n"
"\n"
"  /* ----- URLSearchParams (WHATWG) ----- */\n"
"\n"
"  function URLSearchParamsClass(init) {\n"
"    this._list = [];  /* array of [name, value] pairs preserving insertion order */\n"
"    if (init == null) return;\n"
...
"        this._list.push([k, v]);\n"
"      }\n"
"    } else if (init instanceof URLSearchParamsClass) {\n"
...
"          var e = new TypeError('Each query pair must be an iterable [name, value] tuple');\n"
"          throw e;\n"
"        }\n"
...
"      for (var i = 0; i < keys.length; i++) {\n"
"        this._list.push([keys[i], String(init[keys[i]])]);\n"
"      }\n"
"    }\n"
"  }\n"
"  /* WHATWG-style decode: '+' → space, then percent-decode */\n"
"  function _urlspDecode(s) {\n"
"    s = s.replace(/\\+/g, ' ');\n"
"    try { return decodeURIComponent(s); } catch (e) { return s; }\n"
"  }\n"
"  function _urlspEncode(s) {\n"
"    return encodeURIComponent(String(s)).replace(/%20/g, '+');\n"
"  }\n"
"  URLSearchParamsClass.prototype.get = function(name) {\n"
"    name = String(name);\n"
"    for (var i = 0; i < this._list.length; i++)\n"
"      if (this._list[i][0] === name) return this._list[i][1];\n"
"    return null;\n"
"  };\n"
"  URLSearchParamsClass.prototype.getAll = function(name) {\n"
"    name = String(name);\n"
"    var out = [];\n"
"    for (var i = 0; i < this._list.length; i++)\n"
...
"    return out;\n"
"  };\n"
"  URLSearchParamsClass.prototype.has = function(name) {\n"
"    name = String(name);\n"
"    for (var i = 0; i < this._list.length; i++)\n"
"      if (this._list[i][0] === name) return true;\n"
"    return false;\n"
"  };\n"
"  URLSearchParamsClass.prototype.set = function(name, value) {\n"
"    name = String(name); value = String(value);\n"
"    var firstIdx = -1;\n"
"    var out = [];\n"
"    for (var i = 0; i < this._list.length; i++) {\n"
"      if (this._list[i][0] === name) {\n"
"        if (firstIdx === -1) { firstIdx = out.length; out.push([name, value]); }\n"
"      } else out.push(this._list[i]);\n"
"    }\n"
"    if (firstIdx === -1) out.push([name, value]);\n"
"    this._list = out;\n"
"  };\n"
"  URLSearchParamsClass.prototype.append = function(name, value) {\n"
"    this._list.push([String(name), String(value)]);\n"
"  };\n"
"  URLSearchParamsClass.prototype['delete'] = function(name) {\n"
"    name = String(name);\n"
"    var out = [];\n"
"    for (var i = 0; i < this._list.length; i++)\n"
...
"  };\n"
"  URLSearchParamsClass.prototype.keys = function() {\n"
"    var arr = []; for (var i = 0; i < this._list.length; i++) arr.push(this._list[i][0]);\n"
"    return arr;\n"
"  };\n"
"  URLSearchParamsClass.prototype.values = function() {\n"
"    var arr = []; for (var i = 0; i < this._list.length; i++) arr.push(this._list[i][1]);\n"
"    return arr;\n"
"  };\n"
"  URLSearchParamsClass.prototype.entries = function() {\n"
"    var arr = []; for (var i = 0; i < this._list.length; i++) arr.push([this._list[i][0], this._list[i][1]]);\n"
"    return arr;\n"
"  };\n"
...
"    var parts = [];\n"
"    for (var i = 0; i < this._list.length; i++)\n"
"      parts.push(_urlspEncode(this._list[i][0]) + '=' + _urlspEncode(this._list[i][1]));\n"
"    return parts.join('&');\n"
"  };\n"
...
"      input = rewriteUrls(String(base), input);\n"
"      if (typeof input !== 'string') {\n"
"        var e = new TypeError('Invalid URL: ' + String(url));\n"
"        e.code = 'ERR_INVALID_URL'; throw e;\n"
"      }\n"
"    }\n"
"    var c = components(input);\n"
"    if (!c) {\n"
...
"    this._comp = c;\n"
"    this._input = input;\n"
"    /* searchParams is a URLSearchParams bound to this URL */\n"
"    var raw = c.queryString && c.queryString.raw ? c.queryString.raw : '';\n"
"    this._searchParams = new URLSearchParamsClass(raw);\n"
"  }\n"
"  function _urlGet(name) {\n"
"    return { get: function() {\n"
"      switch (name) {\n"
"        case 'href':     return this._buildHref();\n"
"        case 'origin':   return this._comp.origin || 'null';\n"
"        case 'protocol': return this._comp.scheme ? this._comp.scheme + ':' : '';\n"
"        case 'username': return this._comp.username || '';\n"
"        case 'password': return this._comp.password || '';\n"
"        case 'host':     return this._comp.host + (this._comp.portText ? ':' + this._comp.portText : '');\n"
"        case 'hostname': return this._comp.host || '';\n"
"        case 'port':     return this._comp.portText || '';\n"
"        case 'pathname': return this._comp.fullPath || '';\n"
"        case 'search':   return this._searchParams.toString() ? '?' + this._searchParams.toString() : '';\n"
"        case 'hash':     return this._comp.hash || '';\n"
"      }\n"
"    }, set: function(v) {\n"
"      /* basic setter: rebuild via href reparse for any change */\n"
"      var u = parse(this._buildHref());\n"
"      u[name === 'username' || name === 'password' ? name : name] = v;\n"
"      /* simplest: recompose from parts using format() */\n"
"      var rebuilt = (this._comp.scheme || '') + ':' + (this._comp.authority || '') +\n"
"                    (this._comp.fullPath || '') + (this._comp.queryString.raw ? '?' + this._comp.queryString.raw : '') + (this._comp.hash || '');\n"
"      switch (name) {\n"
"        case 'pathname':   rebuilt = this._buildHref().replace(this._comp.fullPath, String(v)); break;\n"
"        case 'search':\n"
"          var nv = String(v);\n"
"          if (nv && nv.charAt(0) !== '?') nv = '?' + nv;\n"
"          this._searchParams = new URLSearchParamsClass(nv.substring(1));\n"
"          return;\n"
"        case 'hash':\n"
"          var nh = String(v);\n"
"          if (nh && nh.charAt(0) !== '#') nh = '#' + nh;\n"
"          this._comp.hash = nh; return;\n"
"      }\n"
"      /* For other setters, reparse */\n"
"      var nu = components(rebuilt);\n"
"      if (nu) { this._comp = nu;\n"
"        this._searchParams = new URLSearchParamsClass(nu.queryString && nu.queryString.raw ? nu.queryString.raw : '');\n"
"      }\n"
"    }, configurable: true, enumerable: true };\n"
"  }\n"
...
"    var scheme = c.scheme ? c.scheme + ':' : '';\n"
"    var auth = c.username ? c.username + (c.password ? ':' + c.password : '') + '@' : '';\n"
"    var hostPort = c.host + (c.portText ? ':' + c.portText : '');\n"
"    var slashes = c.authority && c.authority.indexOf('//') === 0 ? '//' : '';\n"
"    return scheme + slashes + auth + hostPort + (c.fullPath || '') + (qs ? '?' + qs : '') + (c.hash || '');\n"
"  };\n"
"  URLClass.prototype.toString = function() { return this._buildHref(); };\n"
"  URLClass.prototype.toJSON = function() { return this._buildHref(); };\n"
"  Object.defineProperty(URLClass.prototype, 'href',         _urlGet('href'));\n"
"  Object.defineProperty(URLClass.prototype, 'origin',       _urlGet('origin'));\n"
"  Object.defineProperty(URLClass.prototype, 'protocol',     _urlGet('protocol'));\n"
"  Object.defineProperty(URLClass.prototype, 'username',     _urlGet('username'));\n"
"  Object.defineProperty(URLClass.prototype, 'password',     _urlGet('password'));\n"
"  Object.defineProperty(URLClass.prototype, 'host',         _urlGet('host'));\n"
"  Object.defineProperty(URLClass.prototype, 'hostname',     _urlGet('hostname'));\n"
"  Object.defineProperty(URLClass.prototype, 'port',         _urlGet('port'));\n"
"  Object.defineProperty(URLClass.prototype, 'pathname',     _urlGet('pathname'));\n"
"  Object.defineProperty(URLClass.prototype, 'search',       _urlGet('search'));\n"
"  Object.defineProperty(URLClass.prototype, 'hash',         _urlGet('hash'));\n"
"  Object.defineProperty(URLClass.prototype, 'searchParams', {\n"
"    get: function() { return this._searchParams; }, configurable: true, enumerable: true\n"
"  });\n"
"\n"
"  /* Legacy Url class (mostly a stub) */\n"
"  function LegacyUrl() {}\n"
"\n"
"  return {\n"
"    parse: parse,\n"
...
/* URL.canParse (node 19+) and URL.parse (node 22+) are added to the
 * returned URL class after the inline JS runs.  They live separately
 * so the static class definition above stays readable. */
static const char *url_url_statics_js =
"(function(URLClass) {\n"
"  /* URL.canParse(input[, base]) -> boolean.  Same parsing rules as\n"
"     the constructor; just doesn't throw. */\n"
"  if (typeof URLClass.canParse !== 'function')\n"
"    URLClass.canParse = function(input, base) {\n"
"      try { new URLClass(input, base); return true; }\n"
"      catch (e) { return false; }\n"
"    };\n"
"  /* URL.parse(input[, base]) -> URL | null.  Like the constructor\n"
"     but returns null instead of throwing. */\n"
"  if (typeof URLClass.parse !== 'function')\n"
"    URLClass.parse = function(input, base) {\n"
"      try { return new URLClass(input, base); }\n"
"      catch (e) { return null; }\n"
"    };\n"
"})";
...
static void nodeshim_init_url(duk_context *ctx)
{
    ...
    /* Attach URL.parse / URL.canParse statics. */
    duk_get_prop_string(ctx, -1, "URL");      /* stack: [..., url_module, URLClass] */
    duk_eval_string(ctx, url_url_statics_js); /* stack: [..., url_module, URLClass, statics_fn] */
    duk_insert(ctx, -2);                      /* stack: [..., url_module, statics_fn, URLClass] */
    duk_call(ctx, 1);
    duk_pop(ctx);                             /* discard statics_fn return value */
}
/* ============================================================
...
"\n"
"  /* PerformanceEntry / PerformanceMark / PerformanceMeasure -- plain\n"
"     constructors so user code can `instanceof` them. */\n"
...
"    PerformanceEntry.call(this, name, 'mark', t, 0, opts.detail);\n"
"  }\n"
...
"    PerformanceEntry.call(this, name, 'measure', startTime, duration, detail);\n"
"  }\n"
"  PerformanceMeasure.prototype = Object.create(PerformanceEntry.prototype);\n"
"\n"
"  /* In-memory entry log.  Capped at 150 per type by default (matches\n"
"     node's setResourceTimingBufferSize default).  Cap kept loose; user\n"
"     can clearMarks/clearMeasures to reset. */\n"
"  var _entries = [];\n"
"  var _byName  = {};   /* name -> [entries] for getEntriesByName fast path */\n"
"\n"
...
"      /* opts form: {start, end, duration, detail} */\n"
"      var s = startOrOpts.start;\n"
"      var e = startOrOpts.end;\n"
"      var d = startOrOpts.duration;\n"
"      detail = startOrOpts.detail;\n"
"      if (typeof s === 'string') { var m = _findLastMark(s); s = m ? m.startTime : 0; }\n"
"      if (typeof e === 'string') { var m2 = _findLastMark(e); e = m2 ? m2.startTime : _now(); }\n"
"      if (s !== undefined && e !== undefined) { startTime = s; duration = e - s; }\n"
"      else if (s !== undefined && d !== undefined) { startTime = s; duration = d; }\n"
"      else if (e !== undefined && d !== undefined) { startTime = e - d; duration = d; }\n"
"      else { startTime = 0; duration = _now(); }\n"
"    } else {\n"
"      /* legacy form: measure(name, startMark, endMark) */\n"
"      var sm = startOrOpts !== undefined ? _findLastMark(startOrOpts) : null;\n"
"      var em = endMark    !== undefined ? _findLastMark(endMark)    : null;\n"
"      startTime = sm ? sm.startTime : 0;\n"
"      duration  = (em ? em.startTime : _now()) - startTime;\n"
"    }\n"
"    var m = new PerformanceMeasure(name, startTime, duration, detail);\n"
"    _addEntry(m);\n"
"    return m;\n"
"  }\n"
"\n"
...
"  }\n"
"\n"
"  /* timeOrigin: ms since Unix epoch at process start. Node's contract\n"
"     is that timeOrigin + performance.now() ≈ wall-clock ms. In rampart,\n"
"     performance.now() already returns wall-clock ms (high-res), so we\n"
"     set timeOrigin to module-load wall time — a positive ms-since-epoch\n"
"     value as user code expects. Duration measurements via mark.startTime\n"
...
"  }\n"
"\n"
"  /* Stream-class stubs -- node has Deflate/Inflate/Gzip/Gunzip/etc as\n"
"     transform streams.  Would need real stream module.  Throw clearly. */\n"
"  function _streamStub(name) {\n"
"    return function() {\n"
"      var e = new Error('zlib.' + name + ' streaming class not supported (libdeflate is one-shot only)');\n"
...
"    },\n"
"    /* stream classes — not supported */\n"
"    Deflate:    _streamStub('Deflate'),\n"
"    Inflate:    _streamStub('Inflate'),\n"
"    Gzip:       _streamStub('Gzip'),\n"
"    Gunzip:     _streamStub('Gunzip'),\n"
"    DeflateRaw: _streamStub('DeflateRaw'),\n"
"    InflateRaw: _streamStub('InflateRaw'),\n"
"    Unzip:      _streamStub('Unzip'),\n"
"    createDeflate:    _streamStub('createDeflate'),\n"
"    createInflate:    _streamStub('createInflate'),\n"
"    createGzip:       _streamStub('createGzip'),\n"
"    createGunzip:     _streamStub('createGunzip'),\n"
"    createDeflateRaw: _streamStub('createDeflateRaw'),\n"
"    createInflateRaw: _streamStub('createInflateRaw'),\n"
"    createUnzip:      _streamStub('createUnzip'),\n"
...
"  }\n"
"\n"
"  /* ---------- MessagePort (used by parentPort) ---------- */\n"
"  function _MessagePort(chanRead, chanWrite, lock) {\n"
...
"    this._closed    = false;\n"
"  }\n"
"  _MessagePort.prototype = Object.create(EventEmitter.prototype);\n"
"  _MessagePort.prototype.constructor = _MessagePort;\n"
"\n"
"  /* Validate transferList: accept ArrayBuffer instances or _MessagePort\n"
"     instances; reject anything else with a DataCloneError-style throw.\n"
"     We don't actually zero-copy (rampart.thread always deep-copies);\n"
"     we don't actually detach the buffer (duktape ArrayBuffer has no\n"
"     detach hook).  But we VALIDATE the argument so user code that\n"
"     relies on the type check still throws on misuse.                   */\n"
...
"    for (var i = 0; i < list.length; i++) {\n"
"      var t = list[i];\n"
"      var ok = (t instanceof ArrayBuffer)\n"
"            || (t && t instanceof _MessagePort);\n"
"      if (!ok) {\n"
...
"  }\n"
"\n"
"  _MessagePort.prototype.postMessage = function(msg, transferList) {\n"
"    if (this._closed) throw new Error('MessagePort is closed');\n"
"    _validateTransferList(transferList);\n"
"    this._lock.lock();\n"
"    try {\n"
"      var cur = thread.get(this._chanWrite) || [];\n"
"      cur.push(msg);\n"
"      thread.put(this._chanWrite, cur);\n"
"    } finally {\n"
"      this._lock.unlock();\n"
"    }\n"
"  };\n"
"\n"
"  /* Override .on()/.addListener() so adding a 'message' listener auto-\n"
"     starts the channel subscription, matching node's MessagePort\n"
"     behavior.  Explicit .start() also works (covers .once etc.).       */\n"
"  _MessagePort.prototype.addListener = function(event, listener) {\n"
"    var r = EventEmitter.prototype.addListener.call(this, event, listener);\n"
"    if (event === 'message') this._listen();\n"
"    return r;\n"
"  };\n"
"  _MessagePort.prototype.on = _MessagePort.prototype.addListener;\n"
"  _MessagePort.prototype.prependListener = function(event, listener) {\n"
"    var r = EventEmitter.prototype.prependListener.call(this, event, listener);\n"
"    if (event === 'message') this._listen();\n"
"    return r;\n"
"  };\n"
"  _MessagePort.prototype.once = function(event, listener) {\n"
"    var r = EventEmitter.prototype.once.call(this, event, listener);\n"
"    if (event === 'message') this._listen();\n"
"    return r;\n"
"  };\n"
"\n"
"  _MessagePort.prototype._listen = function() {\n"
"    if (this._sub) return;\n"
"    var self = this, chan = this._chanRead, lock = this._lock;\n"
"\n"
"    /* Drain anything deposited before we subscribed. */\n"
"    lock.lock();\n"
"    var initial = thread.get(chan) || [];\n"
"    if (initial.length) thread.put(chan, []);\n"
"    lock.unlock();\n"
"\n"
"    this._sub = thread.onGet(chan, function() {\n"
"      if (self._closed) return;\n"
"      lock.lock();\n"
"      var msgs = thread.get(chan) || [];\n"
"      if (msgs.length) thread.put(chan, []);\n"
"      lock.unlock();\n"
...
"    this.emit('close');\n"
"  };\n"
"  _MessagePort.prototype.start = function() { this._listen(); };\n"
"  _MessagePort.prototype.ref   = function() { return this; };\n"
"  _MessagePort.prototype.unref = function() { return this; };\n"
"  X.MessagePort = _MessagePort;\n"
"\n"
...
"    var port = new _MessagePort(arg.chanP2W, arg.chanW2P, arg.lock);\n"
"    port._listen();\n"
"    X.parentPort = port;\n"
"  };\n"
"\n"
"  /* ---------- MessageChannel ----------\n"
"     Two _MessagePort instances wired symmetrically: port1.postMessage()\n"
"     fires 'message' on port2 and vice versa.  Both ports use the same\n"
"     lock since the two clipboard slots are read/written by both sides.\n"
"     Cross-thread port transfer (via transferList) is NOT supported:\n"
"     ports created in one thread should be used in that same thread.   */\n"
"  function MessageChannel() {\n"
"    if (!(this instanceof MessageChannel)) return new MessageChannel();\n"
"    var id = _newId();\n"
"    var chanA = '_wt_mc_a_' + id; /* port1 -> port2 */\n"
"    var chanB = '_wt_mc_b_' + id; /* port2 -> port1 */\n"
"    var lock  = new rampart.lock();\n"
"    thread.put(chanA, []);\n"
"    thread.put(chanB, []);\n"
"    /* port1 reads chanB (port2's writes), writes chanA */\n"
"    /* port2 reads chanA (port1's writes), writes chanB */\n"
"    this.port1 = new _MessagePort(chanB, chanA, lock);\n"
"    this.port2 = new _MessagePort(chanA, chanB, lock);\n"
"  }\n"
"  X.MessageChannel = MessageChannel;\n"
"\n"
"  /* ---------- BroadcastChannel ----------\n"
"     Process-wide named pub/sub.  Every BroadcastChannel(name) instance\n"
"     in any thread receives 'message' events from every postMessage()\n"
"     on any other instance with the same name (not itself).\n"
"\n"
"     Implementation: per-name registry in the clipboard at\n"
"     `_bc_reg_<name>` -- an array of `{id, chan}` entries.  Each\n"
"     instance has its own private recv channel `_bc_recv_<name>_<id>`.\n"
"     postMessage iterates the registry under a shared lock and posts\n"
"     to every entry's chan except its own.\n"
"\n"
"     The shared lock lives on `global._wt_bc_lock` -- created once at\n"
"     module init in the parent.  Workers spawned after module load\n"
"     receive a copy of the same lock via rampart's global-copy at\n"
"     spawn (the underlying pthread mutex is shared).               */\n"
...
"    this._closed = false;\n"
"    thread.put(this._recvChan, []);\n"
"\n"
"    /* Add self to the per-name registry. */\n"
"    var lock = global._wt_bc_lock;\n"
"    lock.lock();\n"
"    try {\n"
"      var reg = thread.get(this._regKey) || [];\n"
"      reg.push({id: this._id, chan: this._recvChan});\n"
"      thread.put(this._regKey, reg);\n"
"    } finally {\n"
"      lock.unlock();\n"
"    }\n"
"\n"
"    /* Subscribe to recv channel (drain-on-fire). */\n"
"    var self = this;\n"
"    this._sub = thread.onGet(this._recvChan, function() {\n"
"      if (self._closed) return;\n"
"      lock.lock();\n"
"      var msgs = thread.get(self._recvChan) || [];\n"
"      if (msgs.length) thread.put(self._recvChan, []);\n"
"      lock.unlock();\n"
"      for (var i = 0; i < msgs.length; i++) {\n"
"        self.emit('message', msgs[i]);\n"
"      }\n"
...
"        if (reg[i].id === this._id) continue; /* skip self */\n"
"        var cur = thread.get(reg[i].chan) || [];\n"
"        cur.push(msg);\n"
"        thread.put(reg[i].chan, cur);\n"
"      }\n"
"    } finally {\n"
"      lock.unlock();\n"
"    }\n"
...
"  X.BroadcastChannel = BroadcastChannel;\n"
"\n"
"  return X;\n"
"}";
static void nodeshim_init_worker_threads(duk_context *ctx)
