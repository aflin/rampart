...
"    var s = this._readableState;\n"
"    if (s.flowing === true || s.ended) return this;\n"
"    s.flowing = true;\n"
"    if (!this._reader) this._reader = this._web.getReader();\n"
"    var self = this;\n"
"    function loop() {\n"
"      if (!s.flowing || s.ended || s.destroyed) return;\n"
...
"\n"
"  /* ---------- Writable -----------------------------------------\n"
"     Wraps a WHATWG WritableStream.  Per the spec, .write returns\n"
"     a Promise; Node returns boolean (true = ok, false = backpressure).\n"
"     We approximate by always returning true and firing 'drain' after\n"
"     each microtask. */\n"
...
"          if (self._writeImpl) {\n"
"            try {\n"
"              self._writeImpl.call(self, chunk, 'buffer', function (err) {\n"
...
"          if (self._finalImpl) {\n"
"            try {\n"
"              self._finalImpl.call(self, function (err) {\n"
"                if (err) { reject(err); return; }\n"
"                self._writableState.finished = true;\n"
"                self.emit('finish'); self.emit('close');\n"
"                resolve();\n"
"              });\n"
"            } catch (e) { reject(e); }\n"
"          } else {\n"
...
"      }\n"
"    });\n"
"    self._writer = self._web.getWriter();\n"
"  }\n"
...
"    var self = this;\n"
"    var p = chunk != null ? self._writer.write(chunk) : Promise.resolve();\n"
"    p.then(function () { return self._writer.close(); })\n"
"     .then(function () {\n"
...
"    s.destroyed = true;\n"
"    if (err) { s.errored = err; this.emit('error', err); }\n"
"    try { this._writer.abort(err || new Error('destroyed')); } catch (_) {}\n"
"    this.emit('close');\n"
"    return this;\n"
"  };\n"
...
"    /* Manually init writable side without re-running EventEmitter ctor */\n"
"    opts = opts || {};\n"
"    var self = this;\n"
"    self._writableState = {\n"
"      ended: false, finished: false, destroyed: false, errored: null,\n"
...
"    };\n"
"    self.writable = true;\n"
"    self.allowHalfOpen = opts.allowHalfOpen !== false;\n"
"    self._writeImpl = (typeof opts.write === 'function') ? opts.write : null;\n"
"    self._finalImpl = (typeof opts.final === 'function') ? opts.final : null;\n"
"    self._dWeb = new W_WS({\n"
"      write: function (chunk) {\n"
"        return new Promise(function (resolve, reject) {\n"
"          if (self._writeImpl) {\n"
"            try { self._writeImpl.call(self, chunk, 'buffer', function (err) {\n"
"              if (err) reject(err); else resolve();\n"
"            }); } catch (e) { reject(e); }\n"
"          } else resolve();\n"
"        });\n"
...
"        return new Promise(function (resolve, reject) {\n"
"          if (self._finalImpl) {\n"
"            try { self._finalImpl.call(self, function (err) {\n"
"              if (err) { reject(err); return; }\n"
"              self._writableState.finished = true;\n"
"              self.emit('finish');\n"
"              if (!self.allowHalfOpen) try { self.push(null); } catch (_) {}\n"
"              resolve();\n"
"            }); } catch (e) { reject(e); }\n"
"          } else {\n"
"            self._writableState.finished = true;\n"
...
"      }\n"
"    });\n"
"    self._writer = self._dWeb.getWriter();\n"
"  }\n"
...
"  }\n"
"\n"
"  /* The module object itself is callable for legacy compat: some\n"
"     old code does `stream(...)` to get a passthrough; modern code\n"
"     uses the named exports. */\n"
"  var streamMod = {\n"
"    Readable:    Readable,\n"
"    Writable:    Writable,\n"
"    Duplex:      Duplex,\n"
...
"    pipeline:    pipeline,\n"
"    finished:    finished,\n"
"    /* Promise variants — Node has stream.promises.{pipeline,finished}.\n"
"       Light wrappers. */\n"
"    promises: {\n"
"      pipeline: function () {\n"
"        var args = Array.prototype.slice.call(arguments);\n"
