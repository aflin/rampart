...
HSL rgbToHSL(PIXEL rgb)
{
    ...
    hsl.s = s;
    hsl.l = l;
    hsl.hx = (sin(hsl.h * 2.0 * M_PI) + 1.0) *
             0.5; // convert circle to x,y  2rads in circle, remove negative, scale back to 0-1.0
    hsl.hy = (cos(hsl.h * 2.0 * M_PI) + 1.0) *
             0.5; // convert circle to x,y  2rads in circle, remove negative, scale back to 0-1.0
    return (hsl);
}

// is this pixel in the grayscale?
int isGrayPixel(PIXEL rgb)
{
    HSL hsl;
...
float DeltaE2000a(LAB Lab1, LAB Lab2)
{
    Lab1.L *= 100.0;
    Lab2.L *= 100.0;
    Lab1.a *= 200.0;
    ...
    */
    ...
    if (h2Prime < 0.0)
        h2Prime += 360.0;
    float hBarPrime =
        (fabsf(h1Prime - h2Prime) > 180.0) ? (0.5 * (h1Prime + h2Prime + 360.0)) : (0.5 * (h1Prime + h2Prime));
    float t = 1.0 - 0.17 * cosf(M_PI * (hBarPrime - 30.0) / 180.0) + 0.24 * cosf(M_PI * (2.0 * hBarPrime) / 180.0) +
    ...
    float sL = 1.0 + ((0.015 * (lBarPrime - 50.0) * (lBarPrime - 50.0)) /
                      sqrtf(20.0 + (lBarPrime - 50.0) * (lBarPrime - 50.0)));
    ...
    return (sqrtf((dLPrime / (kL * sL)) * (dLPrime / (kL * sL)) + (dCPrime / (kC * sC)) * (dCPrime / (kC * sC)) +
...
float DeltaE2000(LAB lab1, LAB lab2)
{
    ...
    /* Equation 22 */
    double deltaE =
        sqrt(pow(deltaLPrime / (k_L * S_L), 2.0) + pow(deltaCPrime / (k_C * S_C), 2.0) +
...
static int make_term_codes(CCODES *c)
{
    ...
    if (supports_ansi < 1 && // skip this if we don't support ansi (i.e pipe) and
        !(                   // not forced with one of the CCODE_FLAG_FORCE_TERM_* flags
            ...
            (c->flags & CCODE_FLAG_FORCE_TERM_TRUECOLOR || c->flags & CCODE_FLAG_FORCE_TERM_256 ||
