...
closeRespClient(RESPCLIENT *rcp)
{
  if (rcp)
  {
    ...
    if (rcp->fromBuf)
      rp_redisFree(rcp->fromBuf);

    if (rcp->toBuf)
      rp_redisFree(rcp->toBuf);
...
sendRespBufNeeded(RESPCLIENT *rcp, char *fmt, va_list *argp)
{
  ...
  argSizes = rp_redisCalloc(argCount, sizeof(size_t));
  if (!fmtCopy || !argCount)
  {
  ...
  for (p = fmtCopy; *p;)
  {
    ...
    for (q = p; *q && !isspace(*q); q++)
      ; // find the end of this sequence and terminate it
  ...
  { // allocate bigger than needed by RESPCLIENTBUFSZ for future commands that are approx this size
    rcp->toBuf = rp_redisRealloc(rcp->toBuf, bufNeeded + RESPCLIENTBUFSZ);
    if (!rcp->toBuf)
    {
      rcp->rppFrom->errorMsg = "Memory allocation error in sendRespCommand";
