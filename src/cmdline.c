...
"(function(){\n\
    ...
    var exit=process.exit, utils=rampart.utils, fprintf=utils.fprintf,\n\
        ...
        dateFmt=utils.dateFmt, shell=utils.shell;\n\
    var arg, fullServer=-1, printdefaults=0;\n\
    var wd = utils.getcwd();\n\
    var iam = trim(utils.exec('whoami').stdout);\n\
\n\
    var defaultServerConf = function(){\n\
        return {\n\
            ipAddr:       '127.0.0.1',\n\
            ipv6Addr:     '[::1]',\n\
            bindAll:        false,\n\
...
\n\
    var defaultQuickServerConf = function(){\n\
        return {\n\
            ipAddr:         '127.0.0.1',\n\
            ipv6Addr:       '[::1]',\n\
            bindAll:        false,\n\
...
\n\
    ...
    '--accessLog':      'String. Log file name',\n\
    '--errorLog':       'String. error log file name',\n\
    '--logging':        'Bool.   Whether to log requests and errors',\n\
    ...
    '--dirList':        'Bool.   Whether to provide a directory listing if no index.html is found',\n\
...
\n\
    function writePid(name, pid) {\n\
        ...
        try{fprintf(pidfile, '%s', pid);} catch(e){\n\
            fprintf(stderr,'Warn: could not write pid to %s - %s\\n', pidfile, e);\n\
            return;\n\
        }\n\
        if(iam == 'root') {\n\
            try {\n\
                utils.chown({user:serverConf.user, path:pidfile});\n\
            } catch(e) {}\n\
        }\n\
    }\n\
    var validpids={}\n\
    function getPid(name) {\n\
        ...
        if(typeof ret != 'number' || Number.isNaN(ret) ) {\n\
            if(validpids[pidfile])\n\
                fprintf(serverConf.errorLog, true, '%s - Cannot get %s pid from \\'%s\\'.\\n', dateFmt('%Y-%m-%d-%H-%M-%S'), name, pidfile);\n\
...
\n\
    ...
    if(fullServer==1) {\n\
        ...
        serverConf = defaultQuickServerConf();\n\
    }\n\
\n\
    if(printdefaults) {\n\
...
\n\
        ...
        if(arg.charAt(0)=='-' && arg.charAt(1)=='-') {\n\
            ...
            if(getType(serverConf[arg]) == 'Boolean') {\n\
                if(process.argv[i+1] == 'true' || process.argv[i+1] == 'false')\n\
                    val=process.argv[++i];\n\
                else\n\
                    val=true;\n\
        ...
        {\n\
            fprintf(stderr, '\\'%s\\' is an invalid option, a non-existant root directory or a duplicate root directory.\\n', arg);\n\
            exit(1);\n\
        }\n\
        if(getType(serverConf[arg]) != getType(val)) {\n\
            fprintf(stderr, 'Error: \\'%s\\' expects a %s but got \\'%s\\'\\n', arg, getType(serverConf[arg]), val);\n\
        }\n\
\n\
        serverConf[arg]=val;\n\
    }\n\
...
\n\
    if(shutdown) {\n\
        ...
        if(!res.success)\n\
            fprintf(stderr, 'Failed to kill server using pid file \\'%s\\': %s\\n', res.pidfile, res.error ? res.error: 'unknown error');\n\
        res=killPid('monitor');\n\
...
\n\
    ...
    var bind = [];\n\
    var message = 'Server is at http'+(serverConf.secure?'s':'')+'://localhost:'+serverConf.ipPort+'/';\n\
\n\
    if(serverConf.bindAll) {\n\
        if(!serverConf.ipPort || !serverConf.ipv6Port)\n\
            serr('no ip or ipv6 port specified');\n\
...
\n\
    if(!bind.length)\n\
        serr('No ip addr/port specified');\n\
    if(iam != 'root') {\n\
        if(serverConf.ipPort < 1024)\n\
            serr('Error: script must be started as root to bind to IP port ' + serverConf.ipPort);\n\
        if(serverConf.ipv6Port < 1024)\n\
            serr('Error: script must be started as root to bind to IPv6 port ' + serverConf.ipv6Port);\n\
    }\n\
\n\
    var serverpid;\n\
\n\
    global.notFoundImg='';\n\
    if(stat(wd+'/html/images/not-found.jpg'))\n\
        notFoundImg='<p><img style=\"width:65%\" src=\"/images/not-found.jpg\"></p>'\n\
    if(stat(wd+'/html/images/not-found.gif'))\n\
        notFoundImg='<p><img style=\"width:65%\" src=\"/images/not-found.gif\"></p>'\n\
    if(stat(wd+'/html/images/not-found.png'))\n\
        notFoundImg='<p><img style=\"width:65%\" src=\"/images/not-found.png\"></p>'\n\
\n\
...
\n\
    function start_server(restart){\n\
        ...
        {\n\
            ...
            scriptTimeout:  20.0,\n\
            connectTimeout: 20.0,\n\
            developerMode:  serverConf.developerMode,\n\
...
\n\
        if(serverConf.daemon) { \n\
            sleep(0.5);\n\
            if(!kill(serverpid, 0)) { /* line 350 */\n\
                fprintf(stderr, 'Failed to start webserver\\n');\n\
                exit(1);\n\
            }\n\
            writePid('server', serverpid);\n\
            if(!restart)\n\
            {\n\
...
\n\
    if( (!serverConf.daemon || !serverConf.secure) && fullServer==1 && serverConf.redirPort!=-1){\n\
        fprintf(stderr, 'options --redir[Port] requires --daemon and --secure\\n');\n\
        exit(1);\n\
    }\n\
\n\
...
\n\
        ...
        sleep(0.25);\n\
        if(!kill(rpid, 0)) {\n\
            fprintf(stderr, 'Failed to start redirect webserver\\n');\n\
            exit(1);\n\
        }\n\
        writePid('redir-server', rpid);\n\
        if(!restart)\n\
        {\n\
            printf('Redirect Server has been started.\\n' +\n\
            '    Redirect Server pid is %d\\n', rpid);\n\
        }\n\
    }\n\
\n\
    function checkMonitor()\n\
    {\n\
        if( !(serverConf.logging && serverConf.rotateLogs) && !serverConf.monitor )\n\
            return true; //continue and run server\n\
\n\
        if(fullServer!=1)\n\
        {\n\
            fprintf(stderr, 'options --rotateLogs or --monitor not available with --quickserver\\n');\n\
            exit(1);\n\
        }\n\
\n\
        if(!serverConf.daemon)\n\
        {\n\
            fprintf(stderr, 'options --rotateLogs and --monitor require --daemon\\n');\n\
            exit(1);\n\
        }\n\
\n\
        var gzip = trim ( exec('which','gzip').stdout );\n\
\n\
        var pid = utils.daemon();\n\
        if(pid==-1)\n\
        {\n\
            fprintf(stderr, 'Error, could not fork a new monitor process\\n');\n\
            exit(1);\n\
        }\n\
\n\
        if(pid)\n\
...
\n\
        writePid('monitor',process.getpid());\n\
        if(serverConf.logging && serverConf.rotateLogs) {\n\
            ...
            try {\n\
                ...
                while( startTime < now )\n\
                    startTime = new Date(startTime.getTime() + serverConf.rotateInterval);\n\
            } catch(e) {\n\
