...
static void handle_input(struct repl_history *history)
{
		...
		else
		{
			...
			if (cur_char == '\177')
			{
				...
				if (cursor_pos <= 0)
					continue;
				printf("%s%s%s", "\033[2K", "\033[1G", RP_REPL_PREFIX);
				memmove(line_buffer + cursor_pos - 1, line_buffer + cursor_pos, buffer_end - cursor_pos + 1);
			...
			else
			{
				...
				printf("%s%s%s", "\033[2K", "\033[1G", RP_REPL_PREFIX);
				memmove(line_buffer + cursor_pos + 1, line_buffer + cursor_pos, buffer_end - cursor_pos);
	...
	memcpy(history->buffer[history->size - 1], history->buffer[cur_history_idx], strlen(history->buffer[cur_history_idx]));
}
