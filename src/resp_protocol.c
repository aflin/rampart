...
{
  RESPROTO *rpp=ramisCalloc(1,sizeof(RESPROTO));
  if(rpp)
  {
    rpp->items=ramisCalloc(INITIALRESPITEMS,sizeof(RESPITEM));
...
growRESPItems(RESPROTO *rpp)
{
   ...
   RESPITEM *newItems=ramisRealloc(rpp->items,newSize);
   if(newItems==NULL)
   {
...
{
  int i;
  byte *newBuffer=ramisRealloc(oldBuffer,newSize);
  ...
  else rp->errorMsg="Failed attempt to grow recieve buffer size in respBufRealloc()";
...
parseResProto(RESPROTO *rpp,byte *buf,size_t bufLen,int newBuffer)
{
   ...
   while(p<end)
   {
     ...
     switch(*p)
     {
         ...
         case ':': // could be a floating point or an integer in RAMIS
         {
            if(!parseRespNumber(rpp,p+1,&floatingPoint,&integer))
...
respPrintfLen(char *fmt,va_list *args)
{
  ...
  for(p=fmt;*p;p++)
  {
    if(*p=='%')
    {
      ++p;
      ++arraySize;
      if(!strncmp(p,"lld",3))  {len+=strIntegerLen(va_arg(arg,int64_t));p+=3;}
      else
      if(!strncmp(p,"ld",2))   {len+=strIntegerLen((int64_t)va_arg(arg,long));p+=2;}
      else
      if(*(p+1)=='d')         {len+=strIntegerLen((int64_t)va_arg(arg,int));p++;}
      else
      if(!strncmp(p,"lf",2))   {len+=strDoubleLen(va_arg(arg,double));p+=2;}
      else
      if(*p=='f')             {len+=strDoubleLen(va_arg(arg,double));++p;}
      else
      if(*p=='s')
      {
      ...
      if(*p=='b')  // b is bulk string and has two arguments, first is length, second is 
      {
...
respSendReply(RESPROTO *rpp,FILE *fh)
{
  ...
  for(i=0;i<rpp->nOutItems;i++)
  {
     ...
     {
       case(RESPISNULL):   {fprintf(fh,"%s","$-1\r\n");break;}
       case(RESPISFLOAT):  {fprintf(fh,":%#.*e\r\n",DBL_DECIMAL_DIG-1,item->rfloat);break;}
       case(RESPISINT):    {fprintf(fh,":%lld\r\n",item->rinteger);break;}
       case(RESPISARRAY):  {fprintf(fh,"*%zd\r\n",item->length);break;}
       case(RESPISSTR):
       {
         fputc('+',fh);
       ...
       case(RESPISERRORMSG):   {fprintf(fh,"-%s\r\n",item->loc);break;}
     }
  }
