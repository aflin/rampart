...
enum TokenType {
    ...
    REGEX_PATTERN,
    JSX_TEXT,
    STRING_FRAGMENT_RAW,    // matches $.string_fragment_raw
};
...
bool tree_sitter_javascript_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
    ...
    if (valid_symbols[STRING_FRAGMENT_RAW]) {
      // We get here only after the opening ``` literal matched.
      for (;;) {
        int32_t c = lexer->lookahead;
        if (c == 0) return false;  // EOF before closer -> backtrack
        if (c == '`') {
          // Stop BEFORE backticks so the closing ``` literal can match next.
          lexer->result_symbol = STRING_FRAGMENT_RAW;
          return true;             // empty fragment is allowed
        }
        lexer->advance(lexer, false);  // include char
        lexer->mark_end(lexer);        // extend token
      }
    }
