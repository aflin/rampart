...
// @ts-check
const string_fragment_raw = $ => token.immediate(/[^`\\]+/);
module.exports = grammar({
  ...
  externals: $ => [
    ...
    $.regex_pattern,
    $.jsx_text,
    $.string_fragment_raw,   // raw content (no escapes)
  ],
  ...
  rules: {
    ...
    format_head: $ =>
      // "%" followed by one or more non-whitespace, non-":" and non-"}" chars
      // This keeps the ":" (our separator) and "}" (template end) available to the parser.
      token(seq('%', /[^\s:}]+/)),

    bare_format_substitution: $ => seq(
      // ${%d:x}  where %d is "%"+non-whitespace, x is identifier | number | string
      field('format', $.format_head),
      ':',
      field('arg', $.expression)
    ),

    format_substitution: $ => seq(
      // ${"hi %s":"there"}  two string literals separated by ":"
      field('format', $.string),
      ':',
      field('arg', $.expression)
    ),
    ...
    string: $ => choice(
      // New triple-backtick raw string, still a `string` node:
      seq(
        '"',
        repeat(choice(
      ...
      seq(  
        alias('```', '```'),
        field('text', $.string_fragment_raw),
        alias('```', '```')
      )
    ),
    ...
    template_substitution: $ => seq(
      ...
      choice(
        $._expressions,             // existing JavaScript form
        $.bare_format_substitution, // ${%d:x}
        $.format_substitution       // ${"hi %s":"there"}
      ),
//      $._expressions,
