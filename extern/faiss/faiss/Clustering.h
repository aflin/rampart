...
/** Per-iteration progress callback for k-means clustering.  If set on
 * a ClusteringParameters, Clustering::train_encoded invokes
 * on_iteration() once after every k-means iteration completes (so it
 * fires niter * nredo times per Clustering::train call).  Used by
 * embedders that want to drive a progress meter or interrupt long
 * trainings.  Not owned by ClusteringParameters — caller manages
 * lifetime.
 */
struct ClusteringIterationCallback {
    virtual void on_iteration(
            int iter, int n_iter,
            int redo, int n_redo,
            const ClusteringIterationStats& stats) = 0;
    virtual ~ClusteringIterationCallback() = default;
};
...
struct ClusteringParameters {
    ...
    /// Optional progress callback fired once per k-means iteration.  Not
    /// owned; caller must keep alive for the duration of train_encoded().
    ClusteringIterationCallback* iter_cb = nullptr;
};
